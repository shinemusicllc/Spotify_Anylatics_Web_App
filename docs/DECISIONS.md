...
| Use `docker-mailserver` for the VPS mail stack and let Caddy provision `mail.congmail.top` certificates | The current VPS already uses Caddy on `80/443`; a lean SMTP/IMAP stack that reuses Caddy-issued certs avoids a full reverse-proxy takeover by a heavier suite like mailcow. | High | 2026-03-20 |
| Copy Caddy fullchain into `docker-mailserver` custom-certs instead of reading the Caddy volume directly at runtime | Directly pointing DMS TLS to the shared Caddy cert path left SMTP/IMAP on self-signed certs because the helper process reused stale env values and DMS certificate loading was less predictable than its documented custom-certs path. | High | 2026-03-20 |
| Keep shared VPS Caddy host routes tracked in repo and ignore `deploy/mail/docker-data/` as runtime state | The live VPS had an extra `lush.congmail.top` route that was not in GitHub, while the mail stack's Docker data was polluting git status even though it should never be versioned. | High | 2026-03-24 |
